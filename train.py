...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------#
    #   fp16        是否使用混合精度训练
    #               可减少约一半的显存、需要pytorch1.7.1以上   （可以用true）
    # ---------------------------------------------------------------------#
    fp16 = True
    # -----------------------------------------------------#
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    # model_path = "model_data/deeplab_mobilenetv3_large.pth"
    ...
    Init_Epoch = 0
    Freeze_Epoch = 50
    # Freeze_batch_size = 4      # Xception时采用的4，采用8时显示内存不足
    Freeze_batch_size = 8            # 报显存时可以调小
    # ------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    # ------------------------------------------------------------------#
    UnFreeze_Epoch = 100
    # Unfreeze_batch_size = 3     # Xception时采用的3，采用4时显示内存不足
    ...
    if True:
        ...
        gen = DataLoader(train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True, drop_last=True, collate_fn=deeplab_dataset_collate, sampler=train_sampler)
        gen_val = DataLoader(val_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers, pin_memory=True, drop_last=True, collate_fn=deeplab_dataset_collate, sampler=val_sampler)
